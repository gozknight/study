...
func TestParseComMap(t *testing.T) {
	...
	dict, err := o.Dict()
	assert.Equal(t, nil, err)
	assert.Equal(t, BDICT, dict["user"]._type)
	assert.Equal(t, BLIST, dict["value"]._type)
}
